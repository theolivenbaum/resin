...
Resin is a document database that wishes to be a search engine so it's been paired with a search index that just so happens to represent a vector space, a quite powerful concept if you think about it, any vector space, really, as long as it's a (Sir) IModel, which will still feel quite powerful to some of you. But to others, IModel will get in the way. See "Contribute".
