...
Resin is a document database that wishes to be a search engine so it's been paired with a search index that happens to represent a vector space, a quite powerful concept if you think about it, any vector space, really, as long as it's a (Sir) IModel, which will still feel quite powerful to some of you. But to others, IModel will get in the way. See "Contribute".
...
There is both an in-proc, NHibernate-like API in that there are sessions, a factory, and the notion of a unit of work, as well as __JSON-friendly HTTP API that can be extended to support XML or any other document format__, if you which you are one to fully utilize Asp.Net Core 3 MVC's content negotiating capabilities.
